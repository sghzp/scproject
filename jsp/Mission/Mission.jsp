...
/* String sqsm =null;
if(sqsm1 != null){
	sqsm =  URLDecoder.decode(sqsm1, "UTF-8");
} */
List<MesInfo> arr = null;


	if(sjljh == null){
		arr = new MesInfoDaoImpl().query_fortheMission((String)session.getAttribute("bh"));
	}else{
		MesInfoDaoImpl midi = new MesInfoDaoImpl();
		arr = midi.query_fortheMission2((String)session.getAttribute("bh"), sjljh, version);
	} 
...
<html>
  <head>
	...
	<script>
		...
		//alert(document.frames("topmenu"));
		function query(){
			var sjljh = $("#sjljh").val();
			var version  = $("#version").val();
			var wodeliucheng = $("#wodeliucheng").val();
			var clzk = $("#clzk").val();
			window.location.href="<%=basePath%>/jsp/Mission/Mission.jsp?clzk="+clzk+"&sjljh="+sjljh+"&version="+version+"&wodeliucheng="+wodeliucheng;
		}
	</script>
   ...
   <table width="90%">
   		<tr><td>
		    <table width="100%" border="0" cellpadding="8" cellspacing="1" bgcolor="#dddddd">
			    <tr>
			      	<td><a href="jsp/Mission/Mission.jsp"><input type="button" value="待处理事项" style="height: 25px;width: 100px; background:red;"/></a></td>
			...
			<table>
				...
				<%
					while(it.hasNext()){
						...
						if(temp.getClzt()==0 || temp.getClzt() == 2){
				%>
					<tr>
						<td align="center" height="15" class="td" width="3%" >
							<%if(temp.getClzt()==0){
							%>
								<a href="jsp/Mission/Dealing.jsp?num=<%=temp.getNum()%>">处理</a>
							<%
							} else{
							%>
								<a href="jsp/Mission/Chakan.jsp?num=<%=temp.getNum()%>">处理</a>
							<%
							}%>
