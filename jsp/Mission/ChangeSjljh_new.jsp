...
if(request.getParameter("sjljh") != null && request.getParameter("version") != null){
	hexInfo = new HexInfoDaoImpl().queryby_sjljh_version((String)request.getParameter("sjljh"), (String)request.getParameter("version"));
}
...
shengchengrengonghao();
		...
		function shenqing(){
			...
			if($("#clr").val() == "<%=hexInfo.getScz()%>"){
				alert("处理人不能为上传人！");//这里有个隐藏的小bug，因为hexinfo里没有上传者的工号信息，只能判断姓名是否一样，从而导致重名情况无法判断！
				check = 0;
			}
