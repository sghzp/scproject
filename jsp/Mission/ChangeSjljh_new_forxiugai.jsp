...
<%-- 			if(check == 1){
							form.action="<%=basePath%>/jsp/Mission/ChangeSjljh_deal.jsp";
							form.submit();
						} --%>
			<%-- if($("#bz1").val() != "" && $("#bz2").val() != ""){
			...
			if( check == 1){
				...
				$.post("./PanDuanWtdInfo_Tiqian?sjljh=" + sjljh+"&version="+version+"&oristate="+oristate+"&dststate="+dststate+"&style=0",null,function(data){
						if(data == 1){
							form.action="<%=basePath%>/jsp/Mission/ChangeSjljh_deal_new_forxiugai.jsp";
							form.submit();
						}else{
		...
		window.onload=function(){
			var oristate = "<%=mesInfo.getOristate()%>";
			var oristatezhongwen = "<%=mesInfo.getOristate_zhongwen()%>";
			var dststate = "<%=mesInfo.getDststate()%>";
			var dststatezhongwen = "<%=mesInfo.getDststate_zhongwen()%>";
			$("#oristate").html('<option value='+oristate+'>'+oristatezhongwen+'</option>');
			$("#dststate").html('<option value='+dststate+'>'+dststatezhongwen+'</option>');
		};
	</script>
   ...
   <table width="90%">
   		<tr><td>
		    <table width="100%" border="0" cellpadding="8" cellspacing="1" bgcolor="#dddddd">
			    <tr>
					...
					<!-- <td><a href="jsp/Mission/ChangeSjljh_new.jsp"><input type="button" value="数据零件号状态变更申请" style="height: 25px;width: 160px; background:red;"/></a></td>
		...
		<form name="form" action="" method="post" enctype="multipart/form-data">
			<table bgcolor="#E0EEEE" width="100%" font-size="50">
				...
				<tr>
					<td>申请单号：</td>
					<td><input type="text" name="num" value="<%=mesInfo.getNum()%>" readonly="readonly"/></td>
					<td>申请人：</td>
					<td><input type="text" name="sqr" value="<%=mesInfo.getSqr()%>" readonly="readonly"/></td>
				</tr>
				<tr>
					<td>数据零件号<font color="red">*</font>：</td>
			
						<td><input type="text" name="sjljh" id="sjljh" value="<%=mesInfo.getSjljh()%>" readonly="readonly"/></td>		

					<td>版本号<font color="red">*</font>：</td>

						<td><input type="text" name="version" id="version" value="<%=mesInfo.getVersion()%>" readonly="readonly"/></td>		
					
				</tr>
				<tr>
					<td>审核人<font color="red">*</font>：</td>
					<td><input type="text" name="clr" id="clr" value="<%=mesInfo.getClr()%>" readonly="readonly"/>
					</td>
					<td>审核人工号<font color="red">*</font>：</td>
					<td><input type="text" name="clrgh" id="clrgh" value="<%=mesInfo.getClrgh()%>" readonly="readonly"/>
					</td>
				</tr>
				<tr>					
					<td>原状态：</td>
					...
					<td>
						<select name="dststate" size="1" id="dststate">
							<option value="">请选择</option>
				...
				<tr>
					...
					<td><input type="text" id="bz1" name="bz1" value="<%=mesInfo.getBz1()%>" readonly="readonly"/></td>
					<td>临时状态有效期<font color="red">*(半年以内)</font>：</td>
					<td><input type="text" id="bz2" name="bz2" value="<%=mesInfo.getBz2()%>" readonly="readonly"/></td><!-- onchange="checkdaxiao180(this.value)" -->
				</tr>
				<tr>
					<td>申请说明：</td>
					<td colspan="3"><input type="text" id="sqsm" name="sqsm" width="" style="width: 675px; " value="<%=mesInfo.getSqsm()%>"/></td>
				</tr>
				<tr>
					<td>备注：</td>
					<td colspan="3"><input type="text" name="bz" width="" style="width: 675px; " value="<%=mesInfo.getBz()%>"/></td>
				</tr>
				<tr>
					<td>附件1</td>
					<td><input type="text" name="fj1" id="fj1" readonly="readonly" value="<%=mesInfo.getFJ1()%>"/></td>
				...
				<tr>
					<td>附件2</td>
					<td><input type="text" name="fj2" id="fj2" readonly="readonly" value="<%=mesInfo.getFJ2()%>"/></td>
				...
				<tr>
					<td>附件3</td>
					<td><input type="text" name="fj3" id="fj3" readonly="readonly" value="<%=mesInfo.getFJ3()%>"/></td>
