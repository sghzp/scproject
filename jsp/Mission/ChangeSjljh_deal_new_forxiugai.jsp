...
<%
	...
	String fj3 = null;
                ...
                String style = "0";//0为数据零件号，1为整车信息申请、、 int的
...
System.out.println(clr + "********************");
	...
	MesInfo temp = mesInfoDaoImpl.query_by_num(num);
	...
	mesInfoDaoImpl.updatemesInfo(mesInfo);
	mesInfoDaoImpl.update_clzk(mesInfo, clzk);
	...
	//换为多线程
	Thread t = new Thread(new MailThread(userInfo.getMail(),"您有待处理事项-数据零件号为"+mesInfo.getSjljh()+"-版本号为"+mesInfo.getVersion(),"数据零件号为"+mesInfo.getSjljh()+",版本号为"+mesInfo.getVersion()+",请登录以下网址进行处理。"));
	t.start();
	...
	if (check) {//操作成功
		...
		out.println("window.location='../HexPfile/HexPfile.jsp'");
		out.println("</script>");
	} else {//操作失败
			out.println("<script>");
